 ...
 * @param string $type called from settings / form
 * @since Moodle 3.3
 */
function auth_saml2_display_auth_lock_options($settings, $auth, $userfields, $helptext, $mapremotefields, $updateremotefields,
    $customfields = array(), $type = 'settings') {
    global $DB;

    // Check if this function is called from SAML2 admin settings or from IdP settings form.
    ...
    if ($mapremotefields) {
        ...
        if ($type == 'settings') {
            ...
            $settings->addElement('html', html_writer::tag('h3', get_string('auth_fieldlocks', 'auth')));
            $settings->addElement('html', $helptext);
        }
    }
...
/**
 * Get IdP settings for the selected IdP
 * @param int $id IdP ID.
